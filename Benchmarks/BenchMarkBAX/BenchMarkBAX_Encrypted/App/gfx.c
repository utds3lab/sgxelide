...
BNX_BOOL gfxLoadImage( char *filename, SDL_Surface **img )
{
	...
	*/
	fnamemod = (char *)malloc(strlen(filename)+5);
	memcpy(fnamemod,filename,strlen(filename));
	memcpy(fnamemod+strlen(filename),".txt\0",5);
	...
	*/
	...
	temp = IMG_Load_RW(rwops,1);//1 means it will free rwops for us
	...
	if ( temp != 0 )
	{
		...
		*******/
		...
		//fptr = fopen("PREGARBAGE.png","wb");
		//fwrite(buf, sizeof(uint8_t), flen, fptr);
		//fclose(fptr);

		//fptr = fopen("POSTGARBAGE.png","wb");
		//bytes = (uint8_t *)malloc(flen);
		//bridge_get_from_store(bytes,flen,1);
		//fwrite(bytes, sizeof(uint8_t), flen, fptr);
		//fclose(fptr);
...
BNX_BOOL gfxLoadHelp( char *filename )
{
	...
	//Just hardcode it here, why not
	f = fopen("data/text/encrypted_help.txt","rb");
        fread(&flen,sizeof(flen),1,f);
        ebytes = (uint8_t *)malloc(flen);
        fread(ebytes,sizeof(*ebytes),flen,f);
        fclose(f);
        bridge_decrypt_store(ebytes, flen);

	bridge_get_from_store((uint8_t *)&nLines,sizeof(nLines),0);

	_Gfx.helptxt = 0;
	_Gfx.helptxt = ( BNX_HELPLINE * ) malloc( (nLines + 1) * sizeof ( BNX_HELPLINE ) );

	for( i = 0; i < nLines; i++ ){
        	bridge_get_from_store(_Gfx.helptxt[ i ],cGfxMaxTextLine,i+1);
	}
	...
	bridge_add_to_store(&nLines,sizeof(nLines));
	...
	if ( _Gfx.helptxt != 0 )
	{
		...
		if ( f != 0 )
		{
			...
			{
				bridge_add_to_store(_Gfx.helptxt[ nLines ], cGfxMaxTextLine);
				nLines ++;
			}
			_Gfx.helplin = nLines;
			fclose( f );
			bridge_encrypt_store("data/text/encrypted_help.txt");
			bridge_free_store();
			return BNX_TRUE;
		}
		else
