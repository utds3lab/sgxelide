...
void ocall_write_resource(const char *str, const void *bytes, size_t len){
	...
	fwrite(&len,sizeof(uint32_t),1,outfile);//WHY does this need to be uint32_t?  Why do the old files written (supposedly) using size_t work, but now it doesn't and has to use uint32_t??????
...
sgx_enclave_id_t createEnclave()
{
	...
	int updated = 0;
	/*void * stuff = ENCLAVE_FILE;
	FILE *fp = fopen(ENCLAVE_FILE,"r");
	LPSTR pBuf = (LPSTR)malloc(128);
	int len = 128;
