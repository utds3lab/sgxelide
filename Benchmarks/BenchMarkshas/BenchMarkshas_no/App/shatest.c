...
extern int app_SHA1Reset(SHA1Context *context);
extern int app_SHA224Reset(SHA224Context *context);
extern int app_SHA256Reset(SHA256Context *context);
extern int app_SHA384Reset(SHA384Context *context);

extern int app_SHA512Reset(SHA512Context *context);

extern int app_hmacReset(HMACContext *ctx, enum SHAversion whichSha,
    const unsigned char *key, int key_len);

extern int app_hmacInput(HMACContext *ctx, const unsigned char *text,
    int text_len);

extern int app_hmacFinalBits(HMACContext *ctx,
    const uint8_t bits,
    unsigned int bitcount);
extern int app_hmacResult(HMACContext *ctx, uint8_t *digest);
extern int app_USHAReset(USHAContext *ctx, enum SHAversion whichSha);
extern int app_USHAInput(USHAContext *ctx,
              const uint8_t *bytes, unsigned int bytecount);
extern int app_USHAFinalBits(USHAContext *ctx,
                  const uint8_t bits, unsigned int bitcount);
extern int app_USHAResult(USHAContext *ctx,
  ...
  int printPassFail)
{
 ...
 printf("    Length=%d bytes (%d bits), ", length, length * 8);
  ...
  err = keyarray ? app_hmacReset(&hmac, hashes[hashno].whichSha,
                             keyarray, keylen) :
                   app_USHAReset(&sha, hashes[hashno].whichSha);
  if (err != shaSuccess) {
  ...
  for (i = 0; i < repeatcount; ++i) {
    err = keyarray ? app_hmacInput(&hmac, (const uint8_t *) testarray,
                               length) :
                     app_USHAInput(&sha, (const uint8_t *) testarray,
                               length);
    if (err != shaSuccess) {
  ...
  if (numberExtrabits > 0) {
    err = keyarray ? app_hmacFinalBits(&hmac, (uint8_t) extrabits,
                                   numberExtrabits) :
                     app_USHAFinalBits(&sha, (uint8_t) extrabits,
                                   numberExtrabits);
    if (err != shaSuccess) {
  ...
  err = keyarray ? app_hmacResult(&hmac, Message_Digest) :
                   app_USHAResult(&sha, Message_Digest);
  if (err != shaSuccess) {
  ...
  int printResults, int printPassFail)
{
  ...
  if (skipSpaces)
    while ((c = getc(hashfp)) != EOF) {
      if (!isspace(c)) {
        cc = (unsigned char)c;
        err = keyarray ? app_hmacInput(&hmac, &cc, 1) :
                         app_USHAInput(&sha, &cc, 1);
 ...
 if (err != shaSuccess) {
    ...
    while ((nread = fread(buf, 1, sizeof(buf), hashfp)) > 0) {
      err = keyarray ? app_hmacInput(&hmac, buf, nread) :
                       app_USHAInput(&sha, buf, nread);
      if (err != shaSuccess) {
  ...
  if (bitcount > 0)
    err = keyarray ? app_hmacFinalBits(&hmac, bits, bitcount) :
                   app_USHAFinalBits(&sha, bits, bitcount);
  if (err != shaSuccess) {
    fprintf(stderr, "hashfile(): %s Error %d.\n",
    ...
    int printResults, int printPassFail)
{
  ...
  for (j = 0; j < randomcount; j++) {
    ...
    for (i=3; i<1003; i++) {
      ...
      app_USHAReset(&Mi, hashes[hashno].whichSha);
      app_USHAInput(&Mi, MD[i-3], hashsize);
      app_USHAInput(&Mi, MD[i-2], hashsize);
      app_USHAInput(&Mi, MD[i-1], hashsize);
      /* MDi = SHA(Mi); */
      app_USHAResult(&Mi, MD[i]);
    }
    ...
    int printPassFail)
{
  ...
  for (hashno = hashnolow; hashno <= hashnohigh; hashno++) {
    ...
    app_USHAReset(&usha, hashno);
    app_USHAResult(&usha, Message_Digest);
    err = app_USHAInput(&usha, (const unsigned char *)"foo", 3);
    if (printResults == PRINTTEXT)
      printf ("\nError %d. Should be %d.\n", err, shaStateError);
    ...
    err = app_USHAFinalBits(&usha, 0x80, 3);
    if (printResults == PRINTTEXT)
      printf ("\nError %d. Should be %d.\n", err, shaStateError);
    if ((printPassFail == PRINTPASSFAIL) ||
        ((printPassFail == PRINTFAIL) && (err != shaStateError)))
printf("%s se: %s\n", hashes[hashno].name,
    ...
    err = app_USHAReset(0, hashes[hashno].whichSha);
    if (printResults == PRINTTEXT)
       printf("\nError %d. Should be %d.\n", err, shaNull);
    if ((printPassFail == PRINTPASSFAIL) ||
    ...
    switch (hashno) {
      case SHA1: err = app_SHA1Reset(0); break;
      case SHA224: err = app_SHA224Reset(0); break;
      case SHA256: err = app_SHA256Reset(0); break;
      case SHA384: err = app_SHA384Reset(0); break;
      case SHA512: err = app_SHA512Reset(0); break;
    }
    if (printResults == PRINTTEXT)
       printf("\nError %d. Should be %d.\n", err, shaNull);
    if ((printPassFail == PRINTPASSFAIL) ||
...
#else
int shasmain(int argc, char **argv)
#endif
{
  int i, err;
