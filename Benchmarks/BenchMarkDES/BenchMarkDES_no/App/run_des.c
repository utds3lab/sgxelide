 ...
 */
#include "des.h"
extern void app_generate_key(unsigned char* key);
extern void app_generate_sub_keys(unsigned char* main_key, key_set* key_sets);
extern void app_process_message(unsigned char* message_piece, unsigned char* processed_piece, key_set* key_sets, int mode);
...
int desmain(int argc, char* argv[])
#endif
 {
	...
	if (strcmp(argv[1], ACTION_GENERATE_KEY) == 0) { // Generate key file
		...
		start = clock();
		app_generate_sub_keys(des_key, key_sets);
		finish = clock();
			...
			if (block_count == number_of_blocks) {
				if (process_mode == ENCRYPTION_MODE) {
					...
					if (padding < 8) { // Fill empty data block bytes with padding
 						printf("fill empty data bloack bytes with padding");
						memset((data_block + 8 - padding), (unsigned char)padding, padding);
					}

					app_process_message(data_block, processed_block, key_sets, process_mode);
